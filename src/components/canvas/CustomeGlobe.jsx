...
function WorldMap({ dots = [], lineColor = "#F5F5F5" }) {
  ...
  return (
    <div className={twMerge("w-full aspect-[2/1] rounded-lg relative font-sans")} style={{ backgroundColor: isDark ? "#000" : "#fff" }}>
      <img
        src={`data:image/svg+xml;utf8,${encodeURIComponent(svgMap)}`}
        className={twMerge("h-full w-full pointer-events-none select-none")}
        alt="world map"
        height="400"
        width="800"
        draggable={false}
      />
      <svg ref={svgRef} viewBox="0 0 800 400" className={twMerge("w-full h-full absolute inset-0 pointer-events-none select-none")}>
        {dots.map((dot, i) => {
